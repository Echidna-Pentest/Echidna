...
function targetedCommands(commands, targets, targetId, shell_env) {
  ...
  // Always include AI suggested commands regardless of target selection
  const aiCommands = commands.filter((command) => 
    command.group && command.group.endsWith('-AI')
  );
  if (aiCommands.length > 0) {
    mergedcommands = [...mergedcommands, ...aiCommands];
  }
  
  // Remove duplicates based on command id
  const uniqueCommands = mergedcommands.filter((command, index, self) =>
